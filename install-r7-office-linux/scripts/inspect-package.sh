...
if command -v dpkg-deb >/dev/null 2>&1 && dpkg-deb --info "$PACKAGE" >/dev/null 2>&1; then
    ...
    CONTROL_DIR=$(mktemp -d "$STATE_DIR/evidence/deb-control.XXXXXXXXXX")
    dpkg-deb -e "$PACKAGE" "$CONTROL_DIR"
    for script in preinst postinst prerm postrm; do
        if [ -f "$CONTROL_DIR/$script" ]; then
            printf '\n===== %s =====\n' "$script" >>"$SCRIPTS_FILE"
            cat "$CONTROL_DIR/$script" >>"$SCRIPTS_FILE"
        fi
    done
...
AUTHENTICITY=$(package_authenticity "$PACKAGE")
APPROVALS=()
WARNINGS=()
CLASSIFICATION=no-scripts
if [ -s "$SCRIPTS_FILE" ]; then
    CLASSIFICATION=review-required
    APPROVALS+=(review-package-scripts)
fi
...
EXIT_CODE=0
if [ "$AUTHENTICITY" = invalid-signature ]; then
    STATUS=trust-failed
    NEXT=()
    EXIT_CODE=60
elif [ "$COMPATIBLE" = false ] || [ "$CLASSIFICATION" = unknown-privileged ]; then
