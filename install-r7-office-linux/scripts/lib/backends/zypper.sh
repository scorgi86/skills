...
backend_install_local() { zypper --no-refresh --non-interactive install "$1"; }
backend_install_names() { zypper --no-refresh --non-interactive install "$@"; }
backend_find_provider() {
    local results count
    results=$(zypper --no-refresh --non-interactive what-provides "*/$1" 2>/dev/null |
        awk -F'|' 'NF >= 2 && $1 !~ /^[-+]/ {gsub(/^[ \t]+|[ \t]+$/, "", $2); if ($2 != "Name") print $2}' | sort -u) || { printf 'provider-search-unavailable\n'; return 0; }
    count=$(printf '%s\n' "$results" | sed '/^$/d' | wc -l)
