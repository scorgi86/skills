...
Stage 7 возвращает модель с digest без последующего изменения. Stage 8 остаётся отдельным runner и read-only контрактом; `full_run` автоматически вызывает его после закрытого Stage 7 и завершает состояние после проверки manifest. Кэши source evidence принадлежат одному запросу; форматы digest canonical result и report model остаются разными.

Pipeline владеет подготовкой canonical facts для артефактов: Stage 1 и Stage 2 внутри full-flow возвращают сырые результаты, а writer канонизирует их один раз. Прямые вызовы runner сохраняют подготовленный результат прежнего публичного API. Внутренний признак уже подготовленных facts передаётся отдельным аргументом writer и не читается из пользовательского request.

Для Stage 1 и Stage 2 pipeline также назначает общий дисковый AST-кеш, привязанный к digest `repositoryScope`. Явно заданный `request.ast.cache` имеет приоритет. Кеш не входит в canonical state, lineage или evidence и может быть удалён без потери результата исследования. Внутри одного выполнения стадии один memory-only source snapshot обслуживает collection и canonical confirmation; следующий запуск читает файлы заново.
