...
Для Stage 1 и Stage 2 pipeline также назначает общий дисковый AST-кеш, привязанный к digest `repositoryScope`. Явно заданный `request.ast.cache` имеет приоритет. Кеш не входит в canonical state, lineage или evidence и может быть удалён без потери результата исследования.
