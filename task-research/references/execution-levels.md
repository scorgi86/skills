...
Do not ask for these values unless the user requests a choice, goal execution is unavailable, or the requested behavior conflicts with the defaults. If goal execution is unavailable, fall back to `interactive`, record that fact, and retain `continuous`. A targeted Graph/AST question requires neither selection.
...
4. When the environment does not expose reliable remaining-context information, use conservative estimates from model-visible artifacts and `scripts/cli/src/commands/measure_context.js`; never claim an exact remaining-token count.
