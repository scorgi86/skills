...
const { withFileLockAsync } = require("../file_transaction.js");
...
class StageUnitOfWork {
  ...
  async run() {
    return withFileLockAsync(`${this.journal.file}.lock`, () => this.runLocked());
  }

  async runLocked() {
    const journal = await this.loadOrPrepare();
    ...
    return this.inventoryLock && advancesState ? withFileLockAsync(this.inventoryLock, commit) : commit();
  }
  ...
  async loadOrPrepare() {
    ...
    try {
      await this.prepare(prepared, previous, archived);
