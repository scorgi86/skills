...
const { renameWithRetry, withFileLockAsync } = require("../src/file_transaction.js");
...
test("async file lock remains held until the operation settles", async () => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), "async-lock-"));
  const lock = path.join(directory, "operation.lock");
  let release;
  const pending = withFileLockAsync(lock, () => new Promise((resolve) => { release = resolve; }));
  assert.equal(fs.existsSync(lock), true);
  await assert.rejects(withFileLockAsync(lock, async () => {}), /locked/);
  release("done");
