...
test("real Stage2 keeps orphan ownership blocked after budget projection", async t => {
  ...
  const facts = await require("../src/runner.js").runStage2({ stage: 2, transitionArtifact: previous,
...
test("Stage2 instrumentation preserves the direct and deferred results", async t => {
  const fs = require("node:fs"), path = require("node:path"), os = require("node:os");
  ...
  const ordinary = await require("../src/runner.js").runStage2(request, base);
  const calls = { coverage: 0, budget: 0, prepare: 0 };
  const observed = await require("../src/runner.js").runStage2(request, { ...base,
  ...
  await require("../src/runner.js").runStage2(request, { ...base, deferCanonicalization: true,
    evaluateStage2Coverage(...args) { calls.coverage += 1; return evaluateStage2Coverage(...args); },
    applySafeBudget(...args) { calls.budget += 1; return applySafeBudget(...args); },
    prepareFacts(...args) { calls.prepare += 1; return prepareFacts(...args); }
  });
...
test("one coverage and budget pass preserves Stage2 boundary scenarios", async t => {
  const fs = require("node:fs"), path = require("node:path"), os = require("node:os");
  ...
  for (const scenario of scenarios) {
    ...
    const current = await require("../src/runner.js").runStage2(request, base);
    let gate;
    let coverageCalls = 0;
    let budgetCalls = 0;
    const onePass = await require("../src/runner.js").runStage2(request, { ...base,
