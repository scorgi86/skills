...
test("module entries expose their implementation API and scripts root has a single entry file", async () => {
...
test("public entry imports remain silent and pipeline does not eagerly load runners", async () => {
...
test("explicit command entries and router agree across working directories", async () => {
...
test("state CLI initializes a file and router reads it from a different working directory", async () => {
...
test("root entry reports missing and unknown commands through the existing dispatcher", async () => {
...
test("pipeline validates, runs and advances through InventorySession without the state CLI bridge", async () => {
