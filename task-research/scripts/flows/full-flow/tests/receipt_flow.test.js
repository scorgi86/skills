...
async function execute(f, request, facts) { return await runStagePipeline({ outputRoot: f.outputRoot, stateFile: f.stateFile, request, runner: () => facts }); }
function read(result) { return JSON.parse(fs.readFileSync(result.artifact, "utf8")); }
async function start(f) { const result = await execute(f, { ...f.request, openChecks: [f.requirement.check], checkRequirements: [f.requirement] }, { stage: 0, status: "candidate" }); assert.equal(result.status, "partial"); return read(result); }
async function close(f, prior) {
    ...
    return await execute(f, { ...f.request, checkResolutions: [receipt] }, { stage: 0, status: "candidate", canonicalEvidence: [f.proof] });
}
test("partial retry resolves an original proof ID after canonical evidence remapping", async t => {
    const f = fixture(t), prior = await start(f), result = await close(f, prior), canonical = read(result);
...
test("next stage inherits typed metadata receipt proof and immutable question history", async t => {
    const f = fixture(t), prior = await start(f), closed = await close(f, prior), previous = read(closed), bytes = fs.readFileSync(closed.artifact);
    const result = await execute(f, { ...f.request, stage: 1, transitionArtifact: closed.artifact }, { stage: 1, status: "candidate" });
...
test("same-stage partial omission retains typed question and original origin", async t => {
    const f = fixture(t), first = await start(f);
    const result = await execute(f, f.request, { stage: 0, status: "candidate" }), canonical = read(result);
...
test("typed receipt survives all stage boundaries into the final report and state", async t => {
    const f = fixture(t), first = await start(f), closed = await close(f, first), artifacts = [closed.artifact];
    const proofId = read(closed).facts.find(row => row.kind === "check-resolution").evidenceRefs[0];
    for (let stage = 1; stage <= 6; stage++) {
        const result = await execute(f, { ...f.request, stage, transitionArtifact: artifacts.at(-1) }, { stage, status: "candidate", ...(stage === 6 ? { capabilities: [{ id: "ownership", status: "confirmed", requiredForFinalReport: true, evidenceRefs: [proofId] }] } : {}) });
        artifacts.push(result.artifact);
    }
    const result = await runStagePipeline({ stateFile: f.stateFile, outputRoot: f.outputRoot, request: { ...f.request, stage: 7, priorArtifacts: artifacts, transitionArtifact: artifacts.at(-1), evidenceSelectors: [{ artifact: artifacts.at(-1), limit: 20 }], transition: { "next stage": "8" } } });
...
test("stale inherited proof rejects downstream progression and preserves closed bytes", async t => {
    const f = fixture(t), prior = await start(f), closed = await close(f, prior), bytes = fs.readFileSync(closed.artifact), stateBytes = fs.readFileSync(f.stateFile);
    fs.writeFileSync(f.file, "changed();\n");
    await assert.rejects(execute(f, { ...f.request, stage: 1, transitionArtifact: closed.artifact }, { stage: 1, status: "candidate" }), /source-confirmation|stale|proof|receipt/i);
