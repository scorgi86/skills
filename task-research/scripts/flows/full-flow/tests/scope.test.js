...
test("scope preflight rejects conflict before runner and before transaction resume", async t => {
  const f = fixture(t); let calls = 0;
  f.request.repos = [{ id: "source", path: path.join(f.root, "different") }];
  const runner = () => { calls++; return { stage: 0, status: "candidate" }; };
  await assert.rejects(runStagePipeline({ ...f, runner }), /scope|repos/i);
...
test("pipeline preserves raw scope and supplies execution descriptor for injected runner", async t => {
  const f = fixture(t);
  const raw = f.request.repositoryScope;
  raw.repositories[0].exclusions.push("excluded/**");
  const result = await runStagePipeline({ ...f, runner: () => ({ stage: 0, status: "candidate" }) });
...
test("published real runner preserves per-repository exclusions and transition", async t => {
  ...
  const result = await runStagePipeline({ ...f });
  const canonical = JSON.parse(fs.readFileSync(result.artifact));
...
for (const phase of ["prepared", "published"]) {
  for (const kind of ["missing", "mode", "root", "exclusions"]) {
    test(`scope rejection preserves ${phase} transaction with ${kind} descriptor`, async t => {
      const f = fixture(t);
...
test("real pipeline CLI resumes descriptor-bound publication without executing runner", async t => {
  const f = fixture(t);
