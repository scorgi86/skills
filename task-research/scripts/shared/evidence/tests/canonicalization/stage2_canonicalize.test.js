...
test("legacy confirmation cannot acquire a fragment from a snippet or current source", () => {
  ...
  const base = { repository: "repository-a", file: "source/component.js", line: 1, endLine: 1, excerpt: "const feature = true;", confirmation: { status: "source-confirmed", evidenceRefs: ["source-1"], sourceHash: repo.hash } };
  assert.equal(canonicalizeStage2Candidates([base], { repositoryScope: repo.scope }).evidence[0].confirmation.status, "candidate");
  assert.equal(canonicalizeStage2Candidates([{...base, sourceFragment: "wrong"}], { repositoryScope: repo.scope }).evidence[0].confirmation.status, "candidate");
});
test("source adapter and repeated canonicalization preserve exact confirmed multiline anchors", () => {
  const repo = repository(), { candidatesFromSourceEvidence } = require("../../src/canonicalization/candidate_adapters.js");
  const sourceFragment = "\tconst λ = 1;  \nreturn λ;";
  fs.writeFileSync(repo.file, `${sourceFragment}\n`);
  const confirmation = { status: "source-confirmed", evidenceRefs: ["source-1"], sourceHash: crypto.createHash("sha256").update(fs.readFileSync(repo.file)).digest("hex") };
  const candidates = candidatesFromSourceEvidence({ checks: [{ matches: [{repository: "repository-a", file: "source/component.js", line: 1, endLine: 2, sourceFragment, confirmation}] }] });
  const first = canonicalizeStage2Candidates(candidates, { repositoryScope: repo.scope }).evidence;
  assert.equal(first[0].confirmation.status, "source-confirmed");
  assert.equal(first[0].confirmation.sourceFragment, sourceFragment);
  assert.equal(first[0].range.endLine, 2);
  assert.deepEqual(canonicalizeStage2Candidates(first, {repositoryScope: repo.scope}).evidence[0].confirmation, first[0].confirmation);
});
