...
test("Stage2 producer inherits repository scope and preserves source proof into serialization", async t=>{const f=fixture(t),file=path.join(f.a,"file.js"); const {writeCanonicalTransition}=require(path.join(root,"shared/dto/tests/test_helpers.js")); const {runStage2}=require(path.join(root,"steps/step-2/src/runner.js")); const {createCanonicalStageResult}=require(path.join(root,"shared/artifacts/src/canonical/result.js")); const transitionArtifact=writeCanonicalTransition(f.a,1,{facts:{repositoryScope:f.scope}}); const result=await runStage2({stage:2,transitionArtifact,evidence:{checks:[{id:"check",file,pattern:"feature",confirmation:{status:"source-confirmed",line:2,endLine:2,sourceFragment:"feature();",sourceHash:crypto.createHash("sha256").update(fs.readFileSync(file)).digest("hex")}}]}},{runAstBatch:()=>({results:[],stats:{parseCounts:{}},plan:{compiledBeforeParse:true,lateQueries:0}})}); assert.deepEqual(result.repositoryScope,f.scope); assert.equal(result.canonicalEvidence.find(e=>e.range.startLine===2).confirmation.status,"source-confirmed"); const canonical=createCanonicalStageResult({facts:result}); assert.ok(result.canonicalEvidence.every(e=>canonical.evidenceRefs.includes(e.id))); });
