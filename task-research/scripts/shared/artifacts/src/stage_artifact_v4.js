...
function writeStageArtifact({ outputDir, facts, factsPrepared = false, input, usage, retainRaw = false, metrics = {} }) {
  const root = path.resolve(outputDir), canonicalDir = path.join(root, "canonical");
  const runnerFacts = facts;
  if (!factsPrepared) facts = prepareFacts(facts);
  const canonical = createCanonicalStageResult({ facts, factsPrepared: true, input, usage, metrics });
