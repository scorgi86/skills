...
function runStage0(request, dependencies = {}) {
  ...
  const exclusions = unique(request.exclusions);
  ...
  facts.transition = { schemaVersion: "1.0.0", valid: true, missing: [], fields: { target: facts.target, scope: facts.scope, stage: "0", status: "closed", "confirmed evidence": scanSeeds ? "scope and exact file candidates" : "scope and search plan", "candidate evidence": scans.some((scan) => scan.totalFiles) ? "file-level candidates" : "none", "dictionary/graph/path state": "seed dictionary ready", "skipped/forbidden": exclusions.join(", ") || "none", "open checks": "stages 1-8", "next stage": "1" } };
