...
function scanRepo(repo, terms, exclusions = [], dependencies = {}) {
  ...
  const result = invoke("rg", args, { cwd: repo.path, encoding: "utf8", windowsHide: true, timeout: 30000 });
  ...
  const files = String(result.stdout || "").split(/\r?\n/).filter(Boolean).map((file) => path.resolve(repo.path, file)).sort();
...
function runStage0(request, dependencies = {}) {
  ...
  const scans = scanSeeds ? repos.map((repo) => scanRepo(repo, direct, repo.exclusions, dependencies)) : repos.map((repo) => ({ id: repo.id, path: repo.path, status: "not-run", files: [], totalFiles: 0, reason: "Stage 0 strict mode records a search plan only" }));
...
module.exports = { render, runStage0, scanRepo };
