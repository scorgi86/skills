...
const { projectAst, relevanceScore } = require("../../../shared/output/src/fact_projection");
const { extractTransition } = require("../../../shared/dto/src/extract_stage_transition");
...
const { measureContext } = require("../../../shared/output/src/measure_context");
const { buildReportModel, renderStage2Report } = require("../src/render_stage2_report");
const { runEvidenceChecks } = require("../../../shared/evidence/src/collection/source_evidence.js");
const { buildSourceSlices } = require("../../../shared/evidence/src/source_slice");
const { runStage2 } = require("../src/runner");
const { DEFAULT_BUDGETS, normalizeBudgets } = require("../../../shared/output/src/summary/budget.js");
const { buildStageSummary } = require("../../../shared/output/src/summary/stage_summary.js");
const { sourceProjection } = require("../../../shared/output/src/summary/source_projection.js");
const { compactSummaryAnchors, resolveCompactAnchor } = require("../../../shared/output/src/summary/compaction.js");
const { decodeHumanFields, encodeHumanFields, findInternalReportIdentifiers } = require("../../../shared/output/src/human_report_codec");
const { compareStageRepresentations } = require("../../../shared/output/src/quality_equivalence");
const { evaluateStage2Coverage } = require("../src/coverage_gate");
const { compareStageRuns } = require("../../../shared/output/src/compare_stage_runs");
...
test("bounded source evidence reports total and truncation", () => {
  const output = runEvidenceChecks({ checks: [{ id: "many", file: fixture, pattern: "value", maxMatches: 2 }] });
...
test("source evidence reads a repeated file once per run", () => {
  ...
  try {
    const output = runEvidenceChecks({ checks: [
      { id: "first", file: fixture, pattern: "value" },
...
test("source evidence line cache preserves grouped results for repeated files", () => {
  const shared = runEvidenceChecks({ checks: [
    { id: "first", file: fixture, pattern: "value", maxMatches: 8 },
    { id: "second", file: fixture, pattern: "FeatureValue", maxMatches: 8 },
  ] });
  const separateFirst = runEvidenceChecks({ checks: [{ id: "first", file: fixture, pattern: "value", maxMatches: 8 }] });
  const separateSecond = runEvidenceChecks({ checks: [{ id: "second", file: fixture, pattern: "FeatureValue", maxMatches: 8 }] });
  assert.deepEqual(shared.checks[0], separateFirst.checks[0]);
  assert.deepEqual(shared.checks[1], separateSecond.checks[0]);
});

test("source evidence traverses a compatible scope once", () => {
  let traversals = 0;
  const output = runEvidenceChecks({ checks: [
    { id: "first", file: fixture, pattern: "value" },
    { id: "second", file: fixture, pattern: "RareGlow" },
  ] }, {
    listFiles() { traversals += 1; return [fixture]; },
  });
  assert.equal(output.checks.length, 2);
  assert.equal(traversals, 1);
});

test("source evidence keeps incompatible traversal settings isolated", () => {
  let traversals = 0;
  runEvidenceChecks({ checks: [
    { id: "javascript", file: fixture, pattern: "value", extensions: [".js"] },
    { id: "typescript", file: fixture, pattern: "value", extensions: [".ts"] },
    { id: "case-sensitive-exclusion", file: fixture, pattern: "value", extensions: [".js"], excludeFilePatterns: [/generated/] },
    { id: "case-insensitive-exclusion", file: fixture, pattern: "value", extensions: [".js"], excludeFilePatterns: [/generated/i] },
    { id: "follow-links", file: fixture, pattern: "value", extensions: [".js"], followSymlinks: true },
  ] }, {
    listFiles() { traversals += 1; return [fixture]; },
  });
...
test("source evidence applies maxFiles and allowWideScope per check after shared traversal", () => {
  const secondFile = path.resolve(require("node:path").resolve(__dirname, "../../.."), "../fixtures/prototype/constructors.js");
  let traversals = 0;
  const output = runEvidenceChecks({ checks: [
    { id: "wide", file: fixture, pattern: "value", maxFiles: 1, allowWideScope: true },
    { id: "bounded", file: fixture, pattern: "value", maxFiles: 2 },
  ] }, {
    listFiles() { traversals += 1; return [fixture, secondFile]; },
  });
  assert.equal(traversals, 1);
  assert.equal(output.checks[0].filesScanned, 2);
  assert.equal(output.checks[1].filesScanned, 2);
  assert.throws(() => runEvidenceChecks({ checks: [
    { id: "wide", file: fixture, pattern: "value", maxFiles: 1, allowWideScope: true },
    { id: "blocked", file: fixture, pattern: "value", maxFiles: 1 },
  ] }, {
    listFiles() { return [fixture, secondFile]; },
  }), /scope blocked for blocked/);
});

test("source evidence output is deterministic for shuffled traversal results", () => {
  const secondFile = path.resolve(require("node:path").resolve(__dirname, "../../.."), "../fixtures/prototype/constructors.js");
  const request = { retainAllMatches: true, checks: [{ id: "stable", scope: path.dirname(fixture), pattern: "value", maxMatches: 100 }] };
  const forward = runEvidenceChecks(request, { listFiles() { return [fixture, secondFile]; } });
  const shuffled = runEvidenceChecks(request, { listFiles() { return [secondFile, fixture]; } });
  assert.deepEqual(shuffled, forward);
});

test("retained source evidence preserves every match beyond report cap", () => {
  const output = runEvidenceChecks({ retainAllMatches: true, checks: [{ id: "all", file: fixture, pattern: "value", maxMatches: 1 }] });
...
test("grouped source evidence keeps semantic diversity under a small match cap", () => {
  const output = runEvidenceChecks({ checks: [{
    id: "diverse",
    file: fixture,
    patterns: [{ id: "common", value: "value" }, { id: "rare", value: "setRareValue" }],
    groupBy: "pattern",
    maxMatches: 2,
  }] });
  const check = output.checks[0];
...
test("overlapping evidence patterns do not inflate legacy returned counters", () => {
  const output = runEvidenceChecks({ checks: [{ file: fixture, patterns: ["value", "FeatureValue"], maxMatches: 100 }] });
  const check = output.checks[0];
...
test("required semantic selector can retain a rare tail group", () => {
  const groups = Array.from({ length: 20 }, (_, index) => ({ key: `g-${index}`, owner: "HighFanoutContainer", relation: "method", field: `common-${index}`, target: "FeatureValue", items: 1, evidence: 1, example: { file: fixture, line: index + 1 } }));
  groups.push({ key: "g-rare", owner: "RareOwner", relation: "field-write", field: "rare", target: "FeatureValue", items: 1, evidence: 1, example: { file: fixture, line: 99 } });
...
test("target-aware projection ranks preferred feature groups before broad noise", () => {
  const groups = [
    ...
    { key: "g-neighbor", owner: "FeatureCollection", relation: "field-read", field: "otherValue", target: "unknown", items: 1, evidence: 1, example: { file: fixture, line: 2 } },
    { key: "g-target", owner: "FeatureCollection", relation: "field-write", field: "value", target: "FeatureValue", items: 1, evidence: 1, example: { file: fixture, line: 3 } },
  ];
  const projection = projectAst({ status: "candidate", stats: {}, results: [{ id: "feature", command: "find", status: "candidate", coverage: {}, projectionHints: { preferredTerms: "targetFeature,FeatureValue" }, semanticGroups: groups }] }, { maxGroupsPerQuery: 1 });
  assert.equal(projection.queries[0].groups[0].key, "g-target");
  assert.equal(projection.queries[0].ranking.mode, "target-aware");
  assert.ok(relevanceScore(groups[2], ["FeatureValue"]) > relevanceScore(groups[0], ["FeatureValue"]));
});
...
test("human dictionary round-trip preserves concrete names and rejects unresolved references", () => {
  const summary = {
    stage: 2,
    ast: { queries: [{ id: "feature", groups: Array.from({ length: 40 }, (_, index) => ({
      key: `g-${index}`,
      owner: "FeatureContainer",
      relation: "field-write",
      field: "value",
      target: "FeatureValue",
      firstAnchor: { file: fixture, line: index + 1 },
    })) }] },
    sourceEvidence: [{ id: "source", groups: [{ key: "se-1", label: "FeatureContainer target feature", firstAnchor: { file: fixture, line: 1 } }] }],
  };
...
test("stage 2 renderer expands encoded fields and emits human-readable paths without evidence ids", () => {
  const summary = encodeHumanFields({
    stage: 2,
    status: "candidate",
    ast: { queries: [{ id: "feature", command: "find", status: "candidate", coverage: {}, groups: [{ key: "g-test", owner: "FeatureContainer", relation: "field-write", field: "value", target: "FeatureValue", items: 1, evidence: 1, firstAnchor: { file: "Component.js", line: 10 } }] }] },
    sourceEvidence: [],
    quality: {}, output: {},
  });
  const markdown = renderStage2Report(summary);
  assert.match(markdown, /FeatureContainer\.value -> FeatureValue/);
  assert.match(markdown, /Component\.js:10/);
  assert.equal(findInternalReportIdentifiers(markdown).length, 0);
  assert.doesNotMatch(markdown, /g-test|Evidence ID|ast:feature/);
});

test("quality equivalence preserves digests, counters, required groups and anchors", () => {
  const before = {
    stage: 2,
    ast: { queries: [{ id: "feature", groupDigest: "ast-digest", coverage: { groupsMatched: 2, detailsRequested: 1, detailsReturned: 1, detailsSuppressed: 0 }, groups: [
      { key: "g-required", owner: "FeatureContainer", relation: "field-write", field: "value", target: "FeatureValue", firstAnchor: { file: "Component.js", line: 10 } },
    ] }] },
    sourceEvidence: [{ id: "source", groupDigest: "source-digest", filesScanned: 1, totalMatches: 1, returned: 1, groupsTotal: 1, groups: [{ key: "se-required", label: "source", firstAnchor: { file: "Component.js", line: 10 } }] }],
  };
  const after = encodeHumanFields(before);
  const equivalent = compareStageRepresentations(before, after, { requiredGroups: { feature: [{ key: "g-required" }] }, requiredSourceGroups: { source: ["se-required"] } });
...
test("coverage gate blocks suppression, parse repetition, missing anchors and overflow", () => {
  const valid = {
    stage: 2,
    ast: {
      stats: { parseCounts: { "Component.js": 1 }, failed: 0 },
      plan: { compiledBeforeParse: true, lateQueries: 0 },
      queries: [{ id: "feature", groupDigest: "digest", coverage: { groupsMatched: 1, detailsRequested: 1, detailsReturned: 1, detailsSuppressed: 0, detailEvidenceSuppressed: 0 }, groups: [{ key: "g-1", firstAnchor: { file: "Component.js", line: 10 } }] }],
    },
    sourceEvidence: [{ id: "source", groupDigest: "source", groupsTotal: 1, groups: [{ key: "se-1", firstAnchor: { file: "Component.js", line: 10 } }] }],
  ...
  invalid.ast.stats.parseCounts["Component.js"] = 2;
...
test("stage run comparison returns compact metric deltas and equivalence status", () => {
  const run = {
    ...
    ast: { stats: { parseCounts: { "Component.js": 1 } }, queries: [{ id: "feature", groupDigest: "digest", coverage: { groupsMatched: 1, groupsReturned: 1 }, groups: [] }] },
...
test("human codec and quality CLI tools exchange compact argv-safe JSON", () => {
  ...
  const value = {
    ...
    ast: { stats: { parseCounts: { "Component.js": 1 }, failed: 0 }, plan: { compiledBeforeParse: true, lateQueries: 0 }, queries: [{ id: "feature", command: "find", groupDigest: "digest", coverage: { groupsMatched: 1, groupsReturned: 1, detailsRequested: 0, detailsReturned: 0, detailsSuppressed: 0, detailEvidenceSuppressed: 0 }, groups: [{ key: "g-1", owner: "FeatureContainer", relation: "field-write", field: "value", target: "FeatureValue", firstAnchor: { file: "Component.js", line: 10 } }] }] },
    sourceEvidence: [], output: { bounded: true, overflow: false },
  };
  fs.writeFileSync(beforeFile, JSON.stringify(value));
  const encoded = spawnSync(process.execPath, [path.resolve(require("node:path").resolve(__dirname, "../../.."), "cli/src/commands/human_report_codec.js"), "--input", beforeFile, "--output", encodedFile, "--mode", "encode"], { encoding: "utf8" });
  assert.equal(encoded.status, 0, encoded.stderr || encoded.stdout);
  const equivalent = spawnSync(process.execPath, [path.resolve(require("node:path").resolve(__dirname, "../../.."), "cli/src/commands/quality_equivalence.js"), "--before", beforeFile, "--after", encodedFile], { encoding: "utf8" });
  assert.equal(equivalent.status, 0, equivalent.stderr || equivalent.stdout);
  assert.equal(JSON.parse(equivalent.stdout).ok, true);
  const gate = spawnSync(process.execPath, [path.resolve(require("node:path").resolve(__dirname, "../../.."), "cli/src/commands/coverage_gate.js"), "--input", encodedFile, "--require-plan"], { encoding: "utf8" });
  assert.equal(gate.status, 0, gate.stderr || gate.stdout);
  const compared = spawnSync(process.execPath, [path.resolve(require("node:path").resolve(__dirname, "../../.."), "cli/src/commands/compare_stage_runs.js"), "--before", beforeFile, "--after", encodedFile], { encoding: "utf8" });
...
test("adaptive fitter reaches reserve target without changing full digests or required groups", () => {
  const semanticGroups = Array.from({ length: 20 }, (_, index) => ({ key: `g-${index}`, owner: `Owner${index}`, relation: "field-write", field: index < 2 ? "value" : "noise", target: "FeatureValue", items: 1, evidence: 1, example: { file: fixture, line: index + 1 } }));
...
test("stage budgets are explicit and ignore obsolete top-level output limits", () => {
  const budgets = normalizeBudgets({ maxOutputBytes: 12345, budgets: { summaryBytes: 4096, evidenceBytes: 8192, reportBytes: 16384 } });
  assert.equal(budgets.factsBytes, DEFAULT_BUDGETS.factsBytes);
...
test("summary budget reports explicit overflow without dropping required projection", () => {
  const semanticGroups = Array.from({ length: 20 }, (_, index) => ({ key: `g-${index}`, owner: `Owner${index}`, relation: "field-write", field: "value", target: "FeatureValue", items: 1, evidence: 1, example: { file: fixture, line: index + 1 } }));
  const facts = {
    stage: 2,
    status: "candidate",
...
test("transition extractor returns canonical structured fields", () => {
  const canonical = createCanonicalStageResult({ facts: { stage: 1, status: "closed", transition: { fields: transitionFields(1, { "open checks": "owners" }) } }, input: { stage: 1 } });
  const output = extractTransition(canonical);
  assert.equal(output.fields["open checks"], "owners");
  assert.equal(Object.keys(output.fields).length, 10);
});

test("transition extractor rejects Markdown compatibility input", () => {
...
test("stage 2 renderer is deterministic and cannot promote candidates", () => {
  const facts = {
    stage: 2,
    status: "candidate",
    ast: { results: [{ id: "owners", command: "find", status: "candidate", coverage: { groupsScanned: 1, groupsMatched: 1, groupsReturned: 1, detailsRequested: 1, detailsReturned: 1, detailsSuppressed: 0 }, groups: [{ key: "g-test", owner: "FeatureContainer", relation: "field-write", field: "value", target: "FeatureValue", items: 1, evidence: 1 }] }] },
    sourceEvidence: { checks: [{ id: "source", status: "candidate", filesScanned: 1, totalMatches: 1, returned: 1, truncated: false, matches: [{ file: "Component.js", line: 10, snippet: "this.value = input" }] }] },
...
test("measurement and renderer CLIs run with argv-safe JSON files", () => {
  ...
  const measured = spawnSync(process.execPath, [path.resolve(require("node:path").resolve(__dirname, "../../.."), "cli/src/commands/measure_context.js"), "--request", measurementRequest], { encoding: "utf8" });
  ...
  const rendered = spawnSync(process.execPath, [path.resolve(require("node:path").resolve(__dirname, "../../.."), "cli/src/commands/render_stage2_report.js"), "--input", factsFile, "--output", reportFile, "--stdout", "summary"], { encoding: "utf8" });
...
test("stage 2 summary stdout requires and preserves the full facts artifact", () => {
  ...
  fs.writeFileSync(requestFile, JSON.stringify({ stage: 2, transitionArtifact, maxOutputBytes: 4096, ast: { queries: [{ id: "feature", command: "find", file: fixture, options: { terms: "value" }, groupFilters: { owner: "*" }, includeDetails: true, maxDetails: 500 }] }, evidence: { checks: [{ id: "source", file: fixture, pattern: "value", maxMatches: 2 }] } }));
  const completed = spawnSync(process.execPath, [path.resolve(require("node:path").resolve(__dirname, "../../.."), "cli/src/commands/stage2_runner.js"), "--request", requestFile, "--output", outputFile, "--stdout", "summary"], { encoding: "utf8" });
