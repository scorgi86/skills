...
const { buildBundle, render, runStage0 } = require("./stage0_runner");
...
test("scanning Stage 0 keeps file matches as candidates and bundle output is atomic", () => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), "stage0-runtime-"));
  ...
  const result = buildBundle(facts, bundle);
  assert.equal(fs.existsSync(result.report), true);
  assert.equal(JSON.parse(fs.readFileSync(result.manifest, "utf8")).stage, 0);
  assert.throws(() => buildBundle(facts, bundle), /already exists/);
});
