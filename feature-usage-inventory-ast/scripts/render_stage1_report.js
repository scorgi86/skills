...
function transitionFields(facts) {
  ...
  return {
    ...
    "skipped/forbidden": previous["skipped/forbidden"] || "see limitations in facts bundle",
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (!['--input', '--output', '--stdout'].includes(arg)) throw new Error(`Unknown option: ${arg}`);
...
function main() {
  try {
    ...
    if (options.stdout === "summary") process.stdout.write(`${JSON.stringify({ status: "rendered", output: path.resolve(options.output), bytes: Buffer.byteLength(report) })}\n`);
  } catch (error) {
