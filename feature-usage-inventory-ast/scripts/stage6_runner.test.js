"use strict";
const assert = require("node:assert/strict"); const fs = require("node:fs"); const os = require("node:os"); const path = require("node:path"); const test = require("node:test"); const { buildSummary, runStage6 } = require("./stage6_runner");
test("stage 6 keeps full patch facts and returns bounded summary", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "stage6-")); const transition = path.join(root, "stage5.md"); fs.writeFileSync(transition, ["- target: feature", "- scope: fixture", "- stage: 5", "- status: closed", "- confirmed evidence: paths", "- candidate evidence: renderer", "- dictionary/graph/path state: ready", "- skipped/forbidden: none", "- open checks: reference", "- next stage: 6"].join("\n"));
