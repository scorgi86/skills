# Как работает Task Merge Request Message
...
```text
Используй $task-merge-request-msg и подготовь описание PR для изменений текущей ветки.
```
...
```text
Используй $task-merge-request-msg в режиме Fast и подготовь технический черновик описания PR.
```

Смысловой режим без внешнего провайдера:

```text
Используй $task-merge-request-msg в режиме Semantic. Подготовь информативное описание для ревью.
```

С явной базовой веткой и незакоммиченными изменениями:

```text
Используй $task-merge-request-msg. Подготовь описание текущей ветки относительно origin/v7.4.0, включая worktree и untracked-файлы.
```

С явным контекстом задачи:

```text
Используй $task-merge-request-msg и сохрани описание с артефактами в context/RET-1459.md.
```

Повторный запуск после изменения исходников:

```text
Пересобери описание PR через $task-merge-request-msg по актуальному diff. Не используй предыдущее описание как источник фактов.
```

Глубокий анализ через DeepSeek разрешается только явно:

```text
Используй $task-merge-request-msg в режиме deep и DeepSeek через Polza.ai. Разрешаю передать выбранный diff для read-only анализа.
```
